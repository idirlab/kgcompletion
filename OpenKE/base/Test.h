...
void testHead(REAL *con) {
    ...
    //print results of all test triples
    printf("%ld\t%ld\t%f\t%f\t",l_filter_s+1,l_s+1,1.0/(1+l_filter_s),1.0/(1+l_s));
...
void testTail(REAL *con) {
    ...
    lastTail++;
    //printf("r_filter_s: %ld\t", r_filter_s);
   // printf("r_s:%ld\t", r_s);
    printf("%ld\t%ld\t%f\t%f\n",r_filter_s+1,r_s+1,1.0/(1+r_filter_s),1.0/(1+r_s));
