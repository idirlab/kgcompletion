...
double mrr_s =0., mrr_o=0., mrr_s_raw=0., mrr_o_raw=0. ;
...
std::ofstream log("logfilenew.txt", std::ios_base::app | std::ios_base::out);

    log <<(l_rank/ testTotal+r_rank/ testTotal)/2;
    log << "\t";
    log << (l_tot / testTotal + r_tot / testTotal) /2 * 100;
    log << "\t";
    log << (l_filter_rank/ testTotal + r_filter_rank/ testTotal) /2;
    log << "\t";
    log << (l_filter_tot / testTotal + r_filter_tot / testTotal) /2 *100;
    log << "\t";
    log << (mrr_s/ testTotal + mrr_o/ testTotal)/2;
    log << "\t";
    log << (mrr_s_raw/ testTotal + mrr_o_raw/testTotal)/2;
    log << "\n";
    log.close();
}
