...
def applyrule(test_rel, rules_dir):
    ...
    if np.nonzero(inpo_test[test_rel])[1].any():
        ...
        res_pca = []  # pca confidence
        res_std = []  # standard confidence
        ...
        for itr, rules in enumerate(rulesdic[test_rel]):
            ...
            if rel == rules.split('\t')[0].split(' ')[10]:  # ?x1 R1  ?x2   => ?x3  R2  ?x4
                ...
                if '/m/' in x1 and '/m/' in x4 and x2 == x3:  # c1 R1 a==> a R2 c2
                    x4_id = entity2idx[x4]
                    if x4_id in test_tails:
                        rel_idx = entity2idx[rel1]
                        rel_row, rel_col = np.nonzero(inpo[rel_idx])
                        ...
                        for i in response_pairs:
                            res_pca.append((i[0], i[1], pca_conf, itr))
                            res_std.append((i[0], i[1], std_conf, itr))
                if '/m/' in x2 and '/m/' in x3 and x1 == x4:  # b R1 c1==> c2 R2 b
                    rel_idx = entity2idx[rel1]
                    rel_row, rel_col = np.nonzero(inpo[rel_idx])
                    row, head_col = np.nonzero(inpl[:, rel_col])
                    heads1 = row[np.argsort(head_col)]
                    ...
                    x1_idx = [i for i, item in enumerate(tails1) if item == x2_id]
                    heads1 = heads1[x1_idx]
                    x1_idx = [i for i, item in enumerate(heads1) if item in test_tails]
                    heads1 = heads1[x1_idx]
                    ...
                    for i in response_pairs:
                        res_pca.append((i[0], i[1], pca_conf, itr))
                        res_std.append((i[0], i[1], std_conf, itr))
                if '/m/' in x1 and '/m/' in x3 and x2 == x4:  # c1 R1 b==> c2 R2 b
                    rel_idx = entity2idx[rel1]
                    rel_row, rel_col = np.nonzero(inpo[rel_idx])
                    row, head_col = np.nonzero(inpl[:, rel_col])
                    heads1 = row[np.argsort(head_col)]
                    ...
                    x1_idx = [i for i, item in enumerate(heads1) if item == x1_id]
                    tails1 = tails1[x1_idx]
                    x1_idx = [i for i, item in enumerate(tails1) if item in test_tails]
                    tails1 = tails1[x1_idx]
                ...
                if x4 == x1 == '?b' and '/m' not in x3:  # ?b R1  ?a   => ?a  R2  ?b
                    ...
                    row, tail_col = np.nonzero(inpr[:, rel_col])
                    tails1 = row[np.argsort(tail_col)]
                    x2_idx = [i for i, item in enumerate(heads1) if item in test_tails]
                ...
                if rel == rules.split('\t')[0].split(' ')[16]:
                    ...
                    if len({x1, x2, x3, x4}) == 2:
                        if x6 == x1 == x3 and x5 == x2 == x4:
                            ...
                            x3_idx = [i for i, item in enumerate(heads2) if item in test_tails]
                        ...
                        ###### ?b R1  ?a , ?a R2  ?b   => ?a  R3  ?b ##########
                            ...
                            row, tail_col = np.nonzero(inpr[:, rel_col])
                            tails2 = row[np.argsort(tail_col)]
                            x4_idx = [i for i, item in enumerate(tails2) if item in test_tails]
                        ...
                        ###### ?a R1  ?b , ?b R2  ?a   => ?a  R3  ?b ##########
                            ...
                            row, tail_col = np.nonzero(inpr[:, rel_col])
                            tails1 = row[np.argsort(tail_col)]
                            x2_idx = [i for i, item in enumerate(tails1) if item in test_tails]
                            tails1 = tails1[x2_idx]
                            heads1 = heads1[x2_idx]
                            bname = [idx2entity[i] for i in tails1]
                            aname = [idx2entity[i] for i in heads1]
                    ...
                    if len({x1, x2, x3, x4}) != 2:
                        ...
                        if idx_tail == 1:
                            ...
                            row, tail_col = np.nonzero(inpr[:, rel_col])
                            tails1 = row[np.argsort(tail_col)]
                            tails_idx = [i for i, item in enumerate(tails1) if item in test_tails]
                            tails1 = tails1[tails_idx]
                            heads1 = heads1[tails_idx]
                            ###### ?f R1  ?b , ?f R2  ?a   => ?a  R3  ?b ##########
                            if x1 == x3 and x4 == x5:
                                rel_idx = entity2idx[rel2]
                                rel_row, rel_col = np.nonzero(inpo[rel_idx])
                                row, head_col = np.nonzero(inpl[:, rel_col])
                        ...
                        if idx_tail == 2:  # X1(heads1)--rel1--X2(tails1), X3(heads2)--rel2--X4(tails2)  h_test--rel3--t_test  head_test==X3
                            rel_idx = entity2idx[rel2]
                            rel_row, rel_col = np.nonzero(inpo[rel_idx])
                            row, head_col = np.nonzero(inpl[:, rel_col])
                            heads2 = row[np.argsort(head_col)]
                            row, tail_col = np.nonzero(inpr[:, rel_col])
                            tails2 = row[np.argsort(tail_col)]
                            heads_idx = [i for i, item in enumerate(heads2) if item in test_tails]
                        ...
                        if idx_tail == 3:
                            ...
                            row, tail_col = np.nonzero(inpr[:, rel_col])
                            tails2 = row[np.argsort(tail_col)]
                            tails_idx = [i for i, item in enumerate(tails2) if item in test_tails]
        ...
        print 'pca confidence count'
        evaluate_count(test_rel, rel, res_pca, test_pairs, train_pairs, valid_pairs)
        print 'std confidence count'
        evaluate_count(test_rel, rel, res_std, test_pairs, train_pairs, valid_pairs)
        print 'pca confidence sum'
        evaluate_sum(test_rel, rel, res_pca, test_pairs, train_pairs, valid_pairs)
        print 'std confidence sum'
        evaluate_sum(test_rel, rel, res_std, test_pairs, train_pairs, valid_pairs)


def evaluate_sum(r, rel, res, test_pairs, train_pairs, valid_pairs):
    res_a = sorted(res, key=lambda x: x[1])
    res_groups = {}
    for key, g in groupby(res_a, lambda x: x[1]):
        res_groups[key] = list(g)

    if len(res_groups) == 1:
        for key_res in res_groups:
            res_test = res_groups[key_res]
            res_count = sorted(res_test, key=lambda x: x[0])
            res_c = {}
            final = []
            for key, g in groupby(res_count, lambda x: x[0]):
                res_c[key] = list(g)
            for ii in res_c:
                count = sum(float(item[2]) for item in res_c[ii])
                final.append((res_c[ii][0][0], res_c[ii][0][1], count, res_c[ii][0][3]))

            final_res = sorted(final, key=lambda x: x[2], reverse=True)
            final_pca = [i[2] for i in final_res]
            ...
            for tst in test_pairs:
                if tst in found_res:
                    ...
                    final_pca = [i[2] for i in final_res_filtered]
                    frr = [final_pca.index(i) for i in final_pca]
                    franklist = stats.rankdata(frr, method='dense')
                    found_res_filtered = [(i[0], i[1]) for i in final_res_filtered]
                    res_ind_filtered = found_res_filtered.index(tst)
                    f_res_rank = franklist[res_ind_filtered]
                    if f_res_rank <= 10:
                    ...
                    print '{0} {1} {2} {3} {4} {5} 6}'.format(tst[0], rel, tst[1], rhit10, filtered_hit10, rhit1, fhit1)
                else:
            ...
            if key_res in res_groups:
                ...
                if tst in found_res:
                    ...
                    if fres_rank <= 1:
                        ...
                        fhit1 = 0
                    print '{0} {1} {2} {3} {4} {5} {6}'.format(tst[0], rel, tst[1], rhit10, filtered_hit10, rhit1,
                                                               fhit1)
                else:
                    print '{0} {1} {2} {3} {4} {5} {6}'.format(tst[0], rel, tst[1], 0, 0, 0, 0)
            else:
...
def evaluate_count(r, rel, res, test_pairs, train_pairs, valid_pairs):
    res_a = sorted(res, key=lambda x: x[1])
    res_groups = {}
    for key, g in groupby(res_a, lambda x: x[1]):
        res_groups[key] = list(g)
...
def initialize():
    ...
    with open('./data/FB15k_entity2idx.pkl', 'rb') as f:
        entity2idx = cPickle.load(f)
    with open('./data/FB15k_idx2entity.pkl', 'rb') as f:
