...
public:
    ...
    unordered_map<string, double> evaluate(const Model *model, int truncate) {
        ...
        //raw hits@10
        double hits10_s_raw=0.;
		double hits10_o_raw=0.;
        ...
        for (int i = 0; i < N; i++) {
            ...
            mr_s_raw += rank_s_raw;
            mr_o_raw += rank_o_raw;
			//print link prediction results for each triple
			printf("%.2f\t%.2f\t%f\t%f\t%.2f\t%.2f\t%f\t%f\n",rank_s,rank_s_raw,(1./rank_s),(1./rank_s_raw),rank_o,rank_o_raw,(1./rank_o), (1./rank_o_raw));
            ...
            //raw hits@10
            hits10_s_raw += rank_s_raw <= 10;
            hits10_o_raw += rank_o_raw <= 10;
        }
        ...
        //raw hits@10
        info["hits10_s_raw"] = hits10_s_raw / N;
        info["hits10_o_raw"] = hits10_o_raw / N;
...
void pretty_print(const char* prefix, const unordered_map<string, double>& info) {
	...
	//raw hits@10
	printf("%s  Hits@10\t%.2f\t%.2f\n", prefix, 100*info.at("hits10_s_raw"), 100*info.at("hits10_o_raw"));
...
int main(int argc, char **argv) {
    ...
    if (prediction) {
        ...
        //metrics********************
        double myfmrr = (100*info_te.at("mrr_s")+ 100*info_te.at("mrr_o"))/2;
        double mymrr = (100*info_te.at("mrr_s_raw")+100*info_te.at("mrr_o_raw"))/2;
        double myfmr = (info_te.at("mr_s")+ info_te.at("mr_o"))/2;
        double mymr = (info_te.at("mr_s_raw")+info_te.at("mr_o_raw") )/2;
        double myfhit = (100*info_te.at("hits10_s")+ 100*info_te.at("hits10_o") )/2;
        double myhit = (100*info_te.at("hits10_s_raw")+ 100*info_te.at("hits10_o_raw") )/2;
        std::ofstream log("fb15k_results.txt", std::ios_base::app | std::ios_base::out);
        log << algorithm;
        log << '\t';
        log << mymr;
        log << '\t';
