...
public:
    ...
    void run()
    {
        ...
        FILE* f5 = fopen(("A."+version).c_str(),"r");
        ...
        for (int testid = 0; testid<fb_l.size(); testid+=1)
		{
			...
			//	continue;
			rel_num[rel]+=1;
			//cout<<mid2name[id2entity[h]]<<' '<<mid2name[id2entity[l]]<<' '<<entity2num[rel][h]<<'/'<<e2num[h]<<' '<<entity2num[rel][l]<<'/'<<e2num[l]<<endl;
		...
		cout<<"mrr:"<<(lmrr+rmrr)/(fb_l.size()+fb_r.size())<<endl;
    }
