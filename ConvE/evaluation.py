...
def ranking_and_hits2(model, dev_rank_batcher, vocab, name):
    log.info('')
    log.info('-'*50)
    log.info(name)
    log.info('-'*50)
    ...
    for i, str2var in enumerate(dev_rank_batcher):
        ...
        for i in range(Config.batch_size):
            ...
            target_value2 = pred2[i,e1[i, 0].item()].item()
            #########RAW######################
            target_value1_raw = pred1_raw[i, e2[i, 0].item()].item()
            target_value2_raw = pred2_raw[i, e1[i, 0].item()].item()
    ...
    with open('ConvE237.txt','w')as f:
        for i in range(len(ranks_left)):
            f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(ranks_left[i], ranks_left_raw[i], 1. / ranks_left[i],
                                                              1. / ranks_left_raw[i], ranks_right[i],
                                                              ranks_right_raw[i], 1. / ranks_right[i],
                                                              1. / ranks_right_raw[i]))
    for i in range(10):
        log.info('Hits left @{0}: {1}'.format(i+1, np.mean(hits_left[i])))
        log.info('Hits right @{0}: {1}'.format(i+1, np.mean(hits_right[i])))
    ...
    log.info('Mean reciprocal rank: {0}', np.mean(1./np.array(ranks)))
    #raw results
    log.info('Raw Hits @{0}: {1}'.format(10, np.mean(hits_raw[9])))#raw Hits@10
    log.info('Raw MR: {0}', np.mean(ranks_raw))#raw MR
    log.info('Raw MRR: {0}', np.mean(1. / np.array(ranks_raw)))#raw MRR
