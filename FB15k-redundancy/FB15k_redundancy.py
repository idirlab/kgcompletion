...
def fb15k_duplicate_rels():
    ...
    with open('./results/fb15k_rev.pkl', 'wb') as g:
        cPickle.dump(fb15k_rev, g)
    with open('./results/fb15k_same.pkl', 'wb') as k:
...
def fb15k_actual_rev_rels():
    ...
    for rid in range(14951, 16296):
        ...
        if np.nonzero(inpo[rid])[1].any() == True:
            ...
            rel_row, rel_col = np.nonzero(inpo_valid[rid])  # indexes of test cases that contain 'test_rel'
            row, col = np.nonzero(inpl_valid[:, rel_col])
    ...
    with open('./results/rel2revfb15k_13.pkl', 'wb') as g:
